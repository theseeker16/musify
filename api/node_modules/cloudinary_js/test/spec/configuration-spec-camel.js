...
describe('Configuration', function() {
  return describe("CLOUDINARY_URL environment variable", function() {
    ...
    it("should support api_key, api_secret, cloud_name", function() {
      ...
      expect(config.get("cloudName")).toEqual("cloudname");
      expect(config.get("apiKey")).toEqual("key");
      expect(config.get("apiSecret")).toEqual("secret");
      if (typeof process !== "undefined" && process !== null) {
      ...
      expect(config.get("cloudName")).toEqual("cloudname");
      expect(config.get("apiKey")).toEqual("key");
      return expect(config.get("apiSecret")).toEqual(void 0);
    });
    ...
    return it("should support secure_distribution", function() {
      ...
      expect(config.get("privateCdn")).toBeTruthy();
      expect(config.get("secureDistribution")).toEqual("private_cdn");
      if (typeof process !== "undefined" && process !== null) {
      ...
      expect(config.get("cloudName")).toEqual("cloudname");
      expect(config.get("apiKey")).toEqual("key");
      expect(config.get("apiSecret")).toEqual(void 0);
      expect(config.get("foo")).toEqual("bar");
      expect(config.get("one")).toEqual("two");
      expect(config.get("privateCdn")).toBeTruthy();
      return expect(config.get("secureDistribution")).toEqual("private_cdn");
    });
  });
});

//# sourceMappingURL=configuration-spec-camel.js.map
