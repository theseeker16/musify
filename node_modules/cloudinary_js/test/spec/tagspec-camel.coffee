...
describe "Cloudinary.ImageTag", ->
  ...
  describe ":client_hints", ->
    describe "as option", ->
      includeContext "client_hints", {dpr: "auto", cloudName: "sdk-test", width: "auto", crop: "scale", clientHints: true}
    describe "as global configuration", ->
      beforeEach ->
        cl.config().client_hints = true
      includeContext "client_hints", {dpr: "auto", cloudName: "sdk-test", width: "auto", crop: "scale"}

    describe "false", ->
      it "should use normal responsive behaviour", ->
        cl.config().responsive = true
        image = cl.image('sample.jpg', {width: "auto", crop: "scale", cloudName: "sdk-test", clientHints: false})
        tag = image.outerHTML
        expect(tag).toMatch( /<img.*>/)
    ...
    describe "width", ->
      it "supports auto width", ->
        tag = cl.image( 'sample.jpg', {crop: "scale", dpr: "auto", cloudName: "sdk-test", width: "auto:breakpoints", clientHints: true}).outerHTML
...
#      test_cloudinary_url("video_id", { resourceType: 'video', videoCodec: { codec: 'h264', profile: 'basic', level: '3.1' } },
#                          "#{upload_path}/vc_h264:basic:3.1/video_id", {})
  ...
  it "should generate video tag with fallback", ->
    ...
    expect(new cloudinary.VideoTag("movie", simpleAssign({fallbackContent: fallback}, options)).toHtml()).toBe(
      "<video poster=\"#{expected_url}.jpg\">" +
      "<source src=\"#{expected_url}.webm\" type=\"video/webm\">" +
      "<source src=\"#{expected_url}.mp4\" type=\"video/mp4\">" +
      "<source src=\"#{expected_url}.ogv\" type=\"video/ogg\">" +
      fallback +
      "</video>")
    expect(new cloudinary.VideoTag("movie", simpleAssign({fallbackContent: fallback, sourceTypes: "mp4"}, options)).toHtml()).toEqual(
  ...
  it "should generate video tag with source types", ->
    expected_url = VIDEO_UPLOAD_PATH + "movie"
    expect(cl.video("movie", sourceTypes: ['ogv', 'mp4'])).toEqual(
      "<video poster=\"#{expected_url}.jpg\">" +
      "<source src=\"#{expected_url}.ogv\" type=\"video/ogg\">" +
      "<source src=\"#{expected_url}.mp4\" type=\"video/mp4\">" +
      "</video>")
  ...
  it "should generate video tag with source transformation", ->
    ...
    expect(cl.video("movie", width: 100, crop: "scale", transformation: {'quality': 50}, sourceTransformation: {'ogv': {'quality': 70}, 'mp4': {'quality': 30}})).toEqual(
    ...
    expect(cl.video("movie", width: 100, crop: "scale", transformation: {'quality': 50}, sourceTransformation: {'ogv': {'quality': 70}, 'mp4': {'quality': 30}}, sourceTypes: ['webm', 'mp4'])).toEqual(
      "<video poster=\"#{expected_url}.jpg\" width=\"100\">" +
      "<source src=\"#{expected_url}.webm\" type=\"video/webm\">" +
      "<source src=\"#{expected_mp4_url}.mp4\" type=\"video/mp4\">" +
      "</video>")
  ...
  it "should generate video tag with configurable poster", ->
    ...
    expect(cl.video("movie", poster: expected_poster_url, sourceTypes: "mp4")).toEqual(
      "<video poster=\"#{expected_poster_url}\" src=\"#{expected_url}.mp4\"></video>")

    expected_poster_url = VIDEO_UPLOAD_PATH + "g_north/movie.jpg"
    expect(cl.video("movie", poster: {'gravity': 'north'}, sourceTypes: "mp4")).toEqual(
      "<video poster=\"#{expected_poster_url}\" src=\"#{expected_url}.mp4\"></video>")

    expected_poster_url = DEFAULT_UPLOAD_PATH + "g_north/my_poster.jpg"
    expect(cl.video("movie", poster: {'gravity': 'north', 'public_id': 'my_poster', 'format': 'jpg'}, sourceTypes: "mp4")).toEqual(
      "<video poster=\"#{expected_poster_url}\" src=\"#{expected_url}.mp4\"></video>")

    expect(cl.video("movie", poster: "", sourceTypes: "mp4")).toEqual(
      "<video src=\"#{expected_url}.mp4\"></video>")

    expect(cl.video("movie", poster: false, sourceTypes: "mp4")).toEqual(
      "<video src=\"#{expected_url}.mp4\"></video>")
  describe "attributes", ->
